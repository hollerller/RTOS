	    ...
	    if (pdPASS == xQueueReceive(hao->hqueue, &msg_ui, portMAX_DELAY)){
	        ...
	        switch (msg_ui) {
	          case AO_UI_TYPE_PULSE:
	            ...
	            msg_led = AO_LED_TYPE_SIGNAL;
	            ao_led_send(&ao_leds[0], msg_led);
	            break;

	          case AO_UI_TYPE_SHORT:
	            LOGGER_INFO("button short");
	            msg_led = AO_LED_TYPE_SIGNAL;
	            ao_led_send(&ao_leds[1], msg_led);
	            break;

	          case AO_UI_TYPE_LONG:
	            LOGGER_INFO("button long");
	            msg_led = AO_LED_TYPE_SIGNAL;
	            ao_led_send(&ao_leds[2], msg_led);
	            break;
...
void ao_ui_init(ao_ui_handle_t* hao){
	...
	//Creacion de la tarea
	  BaseType_t status;
	  status = xTaskCreate(task_ao_ui_, "task_ao_ui", 128, (void* const)hao, tskIDLE_PRIORITY +1UL, NULL);
	  while (pdPASS != status)
	  {
	    // error
	  }
